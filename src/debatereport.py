...
def footage_line(meta):
    """One line pointing at the pack's Drive folder, or nothing. Slack carries the
    link, never the bytes (Christopher, 12 Sept 2026)."""
    d = (meta or {}).get("drive") or {}
    if not d.get("url"):
        return ""
    n = sum(1 for f in d.get("files") or [] if str(f.get("name", "")).endswith(".mp4"))
    return "\n\n**Footage:** [%s on Drive](%s) — %d clip%s, subtitles and the cut logs. Parliamentary Recording Unit terms apply to campaign use.\n" % (
        d.get("root", "Debate footage"), d["url"], n, "" if n == 1 else "s")


def canvas_body(meta, report, preview=False, provisional=False, problems=None, selection=None):
    ...
    return head + report.rstrip() + best_speeches_section(selection) + footage_line(meta) + "\n\n---\n\n" + foot


def best_speeches_section(selection, top=None):
    """A 'Best speeches' section for the canvas from selection.json: the chosen
    members, the angle the judge saw, and the passage in their own words, each
    linked to Hansard. Empty string when there is no selection."""
    if not selection or not selection.get("ranked"):
        return ""
    n = top or selection.get("top") or 8
    lines = ["", "## Best speeches", "",
             "*Chosen from the %d members who spoke against; passages verbatim from Hansard. Clips in the pack.*" % len(selection["ranked"]), ""]
    for r in selection["ranked"][:n]:
        who = "[%s](%s)" % (r["name"], r["url"]) if r.get("url") else r["name"]
        lines.append("**%s** — %s" % (who, r.get("angle") or ""))
        if r.get("passage"):
            lines.append("> %s" % r["passage"])
        lines.append("")
    return "\n".join(lines)
...
def canvas_summary(meta, preview=False):
    ...
    d = (meta or {}).get("drive") or {}
    where = "Footage is on Drive: %s" % d["url"] if d.get("url") else "Footage clips are in the pack"
    return ("*%s*\nOur report of %s's debate. %s; Parliamentary Recording "
            "Unit terms apply to campaign use." % (meta.get("title"), meta.get("date"), where))
