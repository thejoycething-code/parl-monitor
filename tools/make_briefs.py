...
OUTPUT FORMAT. Do NOT use JSON. Return each requested field as a marker line
on its own, then the field's prose on the lines beneath it:
...
The marker is three characters, two at-signs around the field id, alone on
its line. Use only the field ids you were asked for, each at most once, in
any order; omit a field entirely rather than writing a marker with nothing
under it. Write quotation marks, apostrophes and paragraph breaks freely:
nothing in the prose needs escaping.

JSON WAS THE WRONG ENVELOPE (21 September 2026). These fields are long prose
that quotes phrases, and a reply that put "good relations" in quotation marks
inside a JSON string ended the string early: the parse failed, and every one
of the twelve fields in the Good Relations brief shipped to Drive as
"[CAMPAIGNER: draft needed]" under an Asana review task that looked
finished. A marker on its own line cannot be broken by the prose it
introduces."""
...
DRAFT_FAILED_NOTE = (
    " -- NARRATIVE DRAFT FAILED: every prose field below is a placeholder, "
    "not a considered blank. Regenerate with: "
    "python3 tools/make_briefs.py --force {0}")

FIELD_MARK = re.compile(r"^\s*@@\s*([a-z_]+)\s*@@\s*$")


def parse_narrative(text, field_ids):
    """{field_id: prose} from a marker-delimited reply.

    Falls back to JSON for a model that answers in the old shape; the
    fallback is insurance, not the contract. Unknown ids are dropped, an
    empty field is dropped, and prose is returned exactly as written.
    """
    ...
    for line in (text or "").splitlines():
        hit = FIELD_MARK.match(line)
        if hit:
            close()
            current, buf = (hit.group(1) if hit.group(1) in wanted else None), []
        elif current:
            buf.append(line)
    close()
    if out:
        return out
    # No markers: an older reply shape. Try JSON, both ways, and give up
    # quietly -- the caller reports the failure.
    ...
    for strict in (True, False):
        try:
            data = json.loads(body, strict=strict)
        except ValueError:
            continue
        if isinstance(data, dict):
            return {k: v for k, v in data.items() if k in wanted and v}
    return {}
...
def draft_narrative(subject, facts, api_key):
    ...
    try:
        ...
        data = parse_narrative(text, [fid for fid, _ in NARRATIVE_FIELDS])
        if not data:
            raise ValueError("no field markers and no parsable JSON in a "
                             "{0}-character reply".format(len(text)))
        out = {}
...
def write_csv(path, subject, fields, rf4_hints, expectation, timeline,
    ...
    with open(path, "w", encoding="utf-8", newline="") as handle:
        ...
        w.writerow([fields.get("campaign_name", subject["title"]) + " (DRAFT)",
                    ...
                    + subject["title"] + DRAFT_FAILED_NOTE.format(subject["slug"])
                    if fields.get("draft_failed") else
                    "Draft generated by parl-monitor. Subject: "
                    + subject["title"],
                    ""])
...
def main():
    ...
    for s in todo:
        ...
        # A brief with NO drafted field is a failure, not a blank template
        # (21 September 2026): the Good Relations brief reached Drive and
        # opened an Asana review task looking finished, and the one log line
        # that mentioned it never named which brief it was. "failed" is a
        # relay trigger word, so this reaches the publish summary.
        draft_failed = bool(api_key) and not drafted
        if draft_failed:
            print("  narrative draft failed for {0}: every field is a "
                  "campaigner placeholder".format(s["slug"]))
        ...
        # A REGENERATED brief keeps the review task and the Drive file it
        # already has (21 September 2026). --force used to open a second
        # Asana task for the same subject and write a row with no
        # drive_file_id, orphaning the published sheet: the next Drive run
        # would adopt it back by name, but the team would be looking at two
        # review tasks for one brief.
        prior = conn.execute("SELECT asana_gid, drive_file_id FROM brief_log "
                             "WHERE slug = ?", (s["slug"],)).fetchone()
        prior_gid = prior["asana_gid"] if prior else None
        prior_drive = prior["drive_file_id"] if prior else None
        approval = {}
        if prior_gid:
            print("  asana: keeping the existing review task {0}".format(prior_gid))
        else:
            try:
        ...
        conn.execute("INSERT OR REPLACE INTO brief_log "
                     ...
                     "drive_file_id) VALUES (?, ?, ?, ?, ?, ?, ?)",
                     (s["slug"], s["title"], today.isoformat(), md_path,
                      "pending", prior_gid or approval.get("task_gid"),
                      prior_drive))
        conn.commit()
