...
def store(conn, rows, body, session, today):
    ...
    for doc, draft, areas in rows:
        conn.execute(
            ...
            "agenda_item, subject, title, amends, instruction, event, kind, dated, "
            "areas, first_seen, last_seen) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
            ...
            "instruction=excluded.instruction, event=excluded.event, "
            "kind=excluded.kind, dated=excluded.dated, "
            "areas=excluded.areas, "
            "last_seen=excluded.last_seen",
            (doc.symbol, body, session, doc.url, doc.size,
             draft.agenda_item, draft.subject, draft.title, draft.amends,
             draft.instruction, draft.event, draft.kind, draft.dated,
