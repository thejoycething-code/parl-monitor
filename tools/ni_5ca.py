...
Placement therefore comes ONLY from recorded votes on human-confirmed
divisions. Questions appear as evidence in Comments and never place anyone --
a question shows activity, not direction, which is the same reason the
Westminster ledger alone cannot fill a column (docs/5ca-notes.md).
...
def load_stance(path=STANCE_PATH):
    """{doc_id: entry} from config/ni_stance.yaml, draft entries included.
...
def place(scored):
    """(column, conflict, decided) from [(stance, date, line)] scored votes.

    Most-directional wins, recency breaks ties -- the suggest_rows rule minus
    KIND_WEIGHT, which cannot matter here because only votes place. A sign
    conflict is flagged and NEVER averaged: +2 and -1 is a flagged +2, not a
    quiet +0.5.
    """
...
def build_rows(conn, area, entries):
    ...
    (scored where the entry is confirmed, listed either way), and questions
    they tabled that matched `area`. Placement from scored votes only.
    """
...
def main():
    ...
    rows = build_rows(conn, area, entries)
    ...
    if drafts:
        print("\n  {0} of {1} meaning line(s) in config/ni_stance.yaml are "
              "still DRAFT.".format(drafts, len(entries)))
    ...
    print("\n  Placement comes only from votes on human-confirmed divisions. "
          "Questions\n  are evidence, not direction. Target is blank: that is "
          "the campaigner's\n  call. This sheet is never posted anywhere.")
    conn.close()
    return 0
